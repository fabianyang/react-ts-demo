...
class SiderMenu extends React.Component<IStoreProps> {
    ...
    get currentRoute() {
        ...
        return props.routerStore ? props.routerStore.location.pathname : "/"
    }
    ...
    goto = ({ key }: { key: string }) => {
        ...
        if (props.routerStore) {
            const { history } = props.routerStore
    ...
    render() {
        ...
        for (const item of menu) {
            if (item.path && item.path === this.currentRoute) {
                currentMenu = item
                break
            }
        }
